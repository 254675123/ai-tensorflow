...
def variable_with_weight_losses(shape, stddev, wl):
    ...
    if wl is not None:
        ...
        # 在机器学习中，不管是分类还是回归任务，都可能因为特征过多而导致过拟合，
        # 一般可以通过减少特征或者惩罚不重要特征的权重来缓解这个问题
        # 但是通常我们并不知道该惩罚哪些特征的权重，而正则化就是帮助我们惩罚特征权重的，
        # 即特征的权重也会成为模型的损失函数的一部分
        # 我们使用w1来控制L2 loss的大小
...
# 两个卷积层之后，是全连接层
# 先把第二个卷积层之后的输出结果flatten，使用tf.reshape函数将每个样本都变成一维向量，
# 使用get_shape函数获取数据扁平化之后的长度
...
# 处理就可以获得最终分类结果（直接比较inference输出的各类的数值大小即可），计算softmax主要是为了计算loss，
# 因此softmax操作整合到后面合理
...
# 到这里就完成了整个网络inference的部分，梳理整个网络结构，设计性能良好的CNN是有一定规律可循的，
# 但是想要针对某个问题设计最合适的网络结构，是需要大量实际摸索的
# 完成模型inference的构建，接下来是计算CNN的loss，这里依然是用cross_entropy，这里我们把softmax的计算和
# cross_entropy的计算合在了一起，即 tf.nn.sparse_softmax_cross_entropy_with_logits()
...
# 前面对图像进行数据增强的操作需要耗费大量CPU时间，因此distorted_inputs使用了16个独立的线程来加速任务，
# 函数内部会产生线程池，在需要使用时会通过TensorFlow queue进行调度启动图片数据增强的线程队列，这里一共使用
# 了16个线程来进行加速，如果不启动线程，那么后续inference以及训练的操作都是无法开始的
...
# 数据增强Data Augmentation在我们的训练中作用很大，它可以给单幅图增加多个副本，提高图片的利用率，
# 防止对某一张图片结构的学习过拟合# 这刚好是利用了图片数据本身的性质，图片的冗余信息量比较大，因此可以制造不同的噪声并
# 让图片依然可以被识别出来。如果神经网络可以克服这些# 噪声并准确识别，那么他的泛化能力必然很好。数据增强大大增加了样本量，
# 而数据量的大小恰恰是深度学习最看重的，深度学习可以在图像识别上领先# 其他算法的一大因素就是它对海量数据的利用效率非常高。
# 其他算法，可能在数据量大到一定程度时，准确率就不再上升了，而深度学习只要提供足够
